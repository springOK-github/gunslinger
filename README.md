...
「受付・進行が人手に依存して詰まる」「同じ相手と再戦しがち」「進捗が見えない」といった運用課題を、データ設計と自動化で解消することを目的に作成しました。

- プレイヤーの状態管理（待機／対戦中／休憩／終了）
- 再戦回避を含む自動マッチング（勝者優先）
- 対戦履歴・勝敗統計の自動更新
- シートの初期化・バックアップまで一連で運用可能

紹介記事（背景・狙い・導入効果）：[「ガンスリンガー方式の大会運営が大変すぎたので、マッチングシステムを自作した話」](https://note.com/springok/n/n10889e7d2317)
...
- **🏁 大会終了**: プレイヤーシートと対戦履歴シートのバックアップ（`(プレイヤー|対戦履歴)_YYYYMMDD_HHmmss`）を作成します。
...
A. 「☕ プレイヤーを休憩させる」を使用してください。復帰時は「↩️ 休憩から復帰させる」で待機状態に戻ります。
