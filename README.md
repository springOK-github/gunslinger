...
- **🏁 大会開始**: カスタムメニューの「🏁 大会開始」を実行するとシートの初期化（プレイヤー / 対戦履歴 / マッチング）を行い、大会運用モードに入ります。実行前に既存データを削除する旨の確認ダイアログが表示されます。
- **🏁 大会終了**: 「🏁 大会終了」を実行すると、対戦履歴シートのバックアップ（`対戦履歴_YYYYMMDD_HHmmss`）を作成します。進行中の対戦がある場合は、ユーザーに確認のうえ強制終了し、プレイヤーを待機状態に戻します（進行中の対戦は履歴に記録されません）。

### メンテナンスモード（MAINTENANCE_MODE）

- 大会終了や保守作業中に自動マッチングが誤って走るのを防ぐため、内部的に `MAINTENANCE_MODE` フラグを利用しています。`endAllActiveMatches()` や `endTournament()` 実行中はこのフラグが設定され、`matchPlayers()` はフラグ検出時に即座に中止します。通常の運用では特に操作は不要ですが、カスタム処理で直接 `matchPlayers()` を呼ぶ場合は意図せずトリガーされないようこの点を意識してください。
...
2. **対戦履歴シート**: 完了した対戦の記録。対戦の時刻、卓番号、両プレイヤーの ID と名前、対戦 ID などを記録し、`勝者名` 列に勝者、`敗者名` 列に敗者を保存します。

3. **マッチングシート**: 現在進行中の対戦。卓番号、両プレイヤーの ID・名前、対戦開始時刻、経過時間を表示します（経過時間は HH:mm:ss 表示）。
