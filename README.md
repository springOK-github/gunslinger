...
- **🏁 大会終了**: 「🏁 大会終了」を実行すると、対戦履歴シートのバックアップ（`対戦履歴_YYYYMMDD_HHmmss`）を作成します。進行中の対戦がある場合は、ユーザーに確認のうえ `endAllActiveMatches()` を実行して強制終了し、履歴へ「大会終了」として追記します（デフォルトでは元データは削除しません）。
